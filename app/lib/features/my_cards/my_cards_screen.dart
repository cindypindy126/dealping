...
class MyCardsScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final credit =
        myProviders.where((p) => p.cardType == 'credit').toList();
    final debit =
        myProviders.where((p) => p.cardType == 'debit').toList();
    final telecom =
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            tooltip: '카드/통신사 추가',
            onPressed: () => _showAddProviderSheet(context, ref, myProviderIds),
          ),
        ],
      ),
      floatingActionButton: myProviders.isEmpty
          ? null
          : FloatingActionButton.extended(
              onPressed: () =>
                  _showAddProviderSheet(context, ref, myProviderIds),
          ...
          : ListView(
              ...
              children: [
                if (credit.isNotEmpty) ...[
                  const _SectionHeader(title: '신용카드'),
                ...
                if (debit.isNotEmpty) ...[
                  const _SectionHeader(title: '체크카드'),
                  const SizedBox(height: 8),
                ...
                if (telecom.isNotEmpty) ...[
                  const _SectionHeader(title: '통신사'),
                  const SizedBox(height: 8),
  ...
  Future<void> _showAddProviderSheet(
...
class _ProviderListItem extends StatelessWidget {
  ...
  const _ProviderListItem(
      {required this.provider, required this.onDelete});
      ...
      confirmDismiss: (_) async {
        return await showDialog<bool>(
          ...
          builder: (ctx) => AlertDialog(
            ...
            content: Text('${provider.issuerName} ${provider.name}을(를) 삭제할까요?'),
            actions: [
              TextButton(
